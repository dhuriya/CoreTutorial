...
export class OrdersDashboard {
  ...
  // Search box text. we match this against a few main column (Order Id,Customer Name,Phone, Tracking Id)
  searchText = '';
  // Selected Status
  // any value from orderStatus Or the special value 'All'
  ...
  currentPage = 1;
  //Modal State
  // when use clicks "View" we store selected row here
  ...
  //When usser clicks a status: 
  //1) update the selectedStatus
  //2) reset the current page to 1
  setStatus(status: OrderStatus | 'All'){
    this.selectedStatus = status;
    this.currentPage = 1;
  }
  // Clears search and resets page when the user clicks the "Clear" button
  clearSearch(){
    this.searchText = '';
    this.currentPage = 1;
  }
  //Runs on typin gin search box. resets page to 1
  ...
  // Total records after applying search + status filters
  get totalRecords(): number{
    return this.filteredOrders.length;
  }
  // Total pages after applying search + status filters
  ...
  //
  // Explanation:
  // - startRecordIndex + pageSize - 1 = last record index for this page
  // - Math.min(..., totalRecords) = don't exceed total records
  get endRecordIndex(): number{
    if(this.totalRecords === 0) return 0;
    return Math.min(this.startRecordIndex + this.pageSize - 1, this.totalRecords);
  }
  ...
  // when user changes "Rows" per page, we reset the current page to 1
  ...
  getDeliveryStatusBadgeClass(deliveryStatus: string): string{
    switch(deliveryStatus){
      case 'NotDispatched': return 'badge bg-secondary';
      case 'InTransit': return 'badge bg-info';
      case 'OutForDelivery': return 'badge bg-warning';
      case 'Delivered': return 'badge bg-success';
      default: return 'badge bg-secondary';
    }
  }
