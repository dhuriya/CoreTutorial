...
/* These are CHILD components used inside StoreContainer's (store-container.html) 
  since this is a Standalone Component (no NgModule), we must explicity import them.
*/
...
@Component({
  ...
  styleUrl: './store-container.css',
  templateUrl: './store-container.html',
})
export class StoreContainer {
  //Cart count show in the header (StoreHeader)
  cartCount = 0;
  // Categories show in the ProductFilter dropdown
  categories: Category[] = ['All', 'Laptop', 'Mobile', 'Accessories'];
  // Current selected category (used for filltering)
  selectedCategory: Category = 'All';
  //Search text entered by user (used for filtering)
  searchText = '';
  //Holds the Product selected when user click 'View
  // if null >= Quick View panel is hidden
  selectedProduct: Product | null = null;
  // In- memory product data (for learing/demo purpose)
 ...
 get filteredProducts(): Product[]{
  ...
  .filter(p => p.name.toLowerCase().includes(this.searchText.trim().toLocaleLowerCase()));
 }
  /*
